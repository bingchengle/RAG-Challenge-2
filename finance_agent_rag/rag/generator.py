...
from finance_agent_rag.rag.retrieval import VectorRetriever, HybridRetriever
from finance_agent_rag.llm.api_requests import APIProcessor
...
class QuestionsProcessor:
    def __init__(
        ...
        if answering_model is None:
            from finance_agent_rag.llm.llm_env import get_default_chat_model
            self.answering_model = get_default_chat_model()
        else:
            self.answering_model = answering_model
        self.parallel_requests = parallel_requests
        self.api_provider = api_provider
        self.chat_processor = APIProcessor(provider=api_provider)
    ...
    def get_answer_for_company(self, company_name: str, question: str, schema: str) -> dict:
        ...
        answer_dict = self.chat_processor.get_answer_from_rag_context(
            question=question,
            rag_context=rag_context,
            schema=schema,
            model=self.answering_model
        )
        self.response_data = self.chat_processor.response_data
    ...
    def process_comparative_question(self, question: str, companies: List[str], schema: str) -> dict:
        ...
        rephrased_questions = self.chat_processor.get_rephrased_questions(
        ...
        comparative_answer = self.chat_processor.get_answer_from_rag_context(
            question=question,
