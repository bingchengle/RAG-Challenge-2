...
import asyncio
from finance_agent_rag.llm.api_request_parallel_processor import process_api_requests_from_file
from openai.lib._parsing import type_to_response_format_param 
import tiktoken
import finance_agent_rag.llm.prompts as prompts
import requests
...
from finance_agent_rag.llm.llm_env import (
    get_chat_client,
    get_default_chat_model,
    get_api_key_for_parallel_processor,
    get_chat_completions_url,
)
from finance_agent_rag.llm.structured_chat import chat_structured_parse
...
class BaseOpenaiProcessor:
    ...
    def send_message(
        ...
        if not is_structured:
            ...
            content, completion = chat_structured_parse(
                self.llm, model, messages, response_format, temperature, seed
            )
        ...
        if usage is not None:
            ...
            self.response_data = {"model": getattr(completion, "model", model), "input_tokens": None, "output_tokens": None}
...
class APIProcessor:
    def __init__(self, provider: Literal["glm", "openai", "ibm", "gemini"] = "glm"):
        self.provider = provider.lower()
        if self.provider in ("glm", "openai"):
            # openai 名称保留为兼容；实际均走智谱 GLM（见 llm_env），不调用 OpenAI 官方
...
class AsyncOpenaiProcessor:
    ...
    async def process_structured_ouputs_requests(
        ...
        if request_url is None:
            request_url = get_chat_completions_url()
        parallel_api_key = get_api_key_for_parallel_processor()
